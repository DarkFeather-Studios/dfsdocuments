...
function Form (_title, _subtitle, _elements, _submittable)
{
	...
	this.loadFromJson = function(obj) {
		...
		this.spectating = obj.spectating;
		...
		this.viewerFirstName = obj.viewerFirstName;
		this.viewerLastName  = obj.viewerLastName;
		this.viewerJobGrade  = obj.viewerJobGrade;
		this.viewerJobLabel  = obj.viewerJobLabel;
		
		let tmp_elements = []
		for (let i=0; i<obj.elements.length; i++)
		{
			let tmpIE = new InputElement(obj.elements[i].label, obj.elements[i].type, obj.elements[i].value === "TIMESTAMP" ? GetNowStamp() : obj.elements[i].value, obj.elements[i].can_be_empty, typeof this.spectating === "undefined" ? obj.elements[i].can_be_edited : false)
			tmp_elements.push(tmpIE);
		}
		this.elements = tmp_elements;

		this.signed = typeof obj.signed === "undefined" ? "" : obj.signed;
		this.submittable = this.signed.length === 0;
	}

	this.sign = function () {
		//alert("HMM?");
		this.signed = this.viewerFirstName + " " + this.viewerLastName + "\n" + this.viewerJobLabel + " - " + this.viewerJobGrade + "\n" + getCreationDate();
		$("#signature_block").addClass("signature_block_signed");
		$("#signature_block").html(this.viewerFirstName + " " + this.viewerLastName + "<br>" + this.viewerJobLabel + " - " + this.viewerJobGrade + "<br>" + getCreationDate());
	}
	...
	this.submit = function() { 
		...
		if (can_submit) {
			$.post('http://dfs_documents/form_submit', json_string);
			activeform.close();
		}
	...
	this.close = function() {
		...
		$.post('http://dfs_documents/form_close', JSON.stringify({}));
	}
}
...
//	//24 characters are needed from `d` (now), then we call .toString()
