...
function OpenNewJobFormMenu()
    ClearMenu()
    PlayerData = ESX.GetPlayerData()
    local sector = exports.dfs_PoliceJob:IsCop() and "police" or PlayerData.job.name
    if DOCUMENT_FORMS[sector] ~= nil then
        for i=1, #DOCUMENT_FORMS[sector], 1 do
            Menu.addButton(DOCUMENT_FORMS[sector][i].headerTitle, "CreateNewForm", DOCUMENT_FORMS[sector][i])
        end
    end
    Menu.addButton(_U('close_bt'), "CloseMenu", nil)
    Menu.hidden = false
...
function OpenMyDocumentsMenu()
    ...
    for i=#USER_DOCUMENTS, 1, -1 do
        ...
        Menu.addButton(date_created .. (USER_DOCUMENTS[i].data.xotikTitle and #USER_DOCUMENTS[i].data.xotikTitle > 0 and USER_DOCUMENTS[i].data.xotikTitle or USER_DOCUMENTS[i].data.headerTitle), "OpenFormPropertiesMenu", USER_DOCUMENTS[i])
    end
    Menu.addButton(_U('close_bt'), "CloseMenu", nil)
    Menu.hidden = false
end
...
function OpenFormPropertiesMenu(aDocument)
    ...
    Menu.addButton("Rename Form", "RenameDocument", aDocument)
...
function RenameDocument(aDocument)
    AddTextEntry("DFS_DOCS_RENAME", "Rename This "..aDocument.data.headerTitle)
    DisplayOnscreenKeyboard(1, "DFS_DOCS_RENAME", "P2", aDocument.data.headerTitle or "", "", "", "", 32) --First "" is defualt, rest ar ontop
    while UpdateOnscreenKeyboard() == 0 do
        DisableAllControlActions(0)
        Wait(0)
    end
    aDocument.data.xotikTitle = GetOnscreenKeyboardResult()
    exports.dfs:TriggerServerCallback("dfs_documents:UpdateDocument", function()
        GetAllUserForms()
    end, aDocument)
end
...
function CreateNewForm(aDocument)
    ...
    ESX.TriggerServerCallback('esx_documents:getPlayerDetails', function (cb_player_details)
        if cb_player_details ~= nil then
            ...
            aDocument.viewerFirstName = cb_player_details.firstname
            aDocument.viewerLastName = cb_player_details.lastname
            aDocument.viewerJobLabel = PlayerData.job.label
            aDocument.viewerJobGrade = PlayerData.job.grade_label
...
function ViewDocument(aDocument)
    ...
    if not aDocument.signed or #aDocument.signed == 0 then
        aDocument.viewerFirstName = MyIdentity.FirstName
        aDocument.viewerLastName = MyIdentity.LastName
        aDocument.viewerJobLabel = PlayerData.job.label
        aDocument.viewerJobGrade = PlayerData.job.grade_label 
    end
...
RegisterNetEvent("dfs_documents:ForceRefresh")
AddEventHandler("dfs_documents:ForceRefresh", function()
    GetAllUserForms()
    --print("Refreshed.")
end)
...
RegisterNUICallback('form_submit', function(data, cb)
    ...
    CloseMenu()
    data.xotikTitle = data.xotikTitle or ""
    ESX.TriggerServerCallback('esx_documents:submitDocument', function (cb_form)
        --print(tostring(data.spectating))
        if cb_form ~= nil and not data.spectating then
